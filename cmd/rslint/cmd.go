...
func printDiagnosticDefault(d rule.RuleDiagnostic, w *bufio.Writer, comparePathOptions tspath.ComparePathsOptions) {
	...
	// Calculate line positions manually (LineMap() method was removed)
	lineMap := []int{0}
	for i, c := range text {
		if c == '\n' {
	...
	if codeboxEndLine == len(lineMap)-1 {
		codeboxEndColumn = len(text) - lineMap[len(lineMap)-1]
	} else {
		codeboxEndColumn = lineMap[codeboxEndLine+1] - lineMap[codeboxEndLine] - 1
	}
	codeboxEnd := lineMap[codeboxEndLine] + codeboxEndColumn
